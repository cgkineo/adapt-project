...
class Translate {
   ...
   * @param {Object} options
   * @param {boolean} options.includeFieldContext whether to include field context such as title and description in the output, where available. Only applies to formats that support this (e.g. csv and xliff) and schema versions 1.0.0 and above.
   * @returns {Translate}
   */
  async export ({ includeFieldContext = false } = {}) {
    const schemas = new Schemas({ framework: this.framework, includedFilter: this.includedFilter, sourcePath: this.sourcePath, schemasConfig: this.framework.schemasConfig })
    schemas.load()

    const isSchemaVersionGte1 = this.framework.isSchemaVersionGte('1.0.0')
    ...
    language.getAllFileItems().forEach(({ file, item }) => {
      ...
      const translatablePaths = applicableSchemas.getTranslatablePaths(item)
      ...
      function recursiveJSONProcess (data, level, path, lookupPath, id, file, component) {
        ...
        if (isSchemaVersionGte1) {
          const translatable = translatablePaths.find(p => p.path === lookupPath)
          if (data && translatable) {
            ...
            if (includeFieldContext) {
              row.title = translatable.title
              row.description = translatable.description
            }
            exportTextData.push(row)
          }
        } else {
...
${entries.map(item => {
    ...
    return `    <trans-unit id="${item.id}${item.path}">
      <source>${value}</source>
      <target>${value}</target>
      ${isSchemaVersionGte1 && includeFieldContext ? `<title>${item.title}</title>` : ''}
      ${isSchemaVersionGte1 && includeFieldContext ? `<description>${item.description}</description>` : ''}
    </trans-unit>
`
...
`
    ...
    const outputGroupedByFile = exportTextData.reduce((prev, current) => {
      ...
      if (isSchemaVersionGte1 && includeFieldContext) {
        prev[current.file].push([`${current.file}/${current.id}${current.path}`, current.value, current.title, current.description])
      } else {
