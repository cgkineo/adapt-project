 ...
 * @typedef {import('../Schemas')} Schemas
...
class Schema {
   ...
   * @param {boolean} options.useDefaults Whether to apply defaults to the data during validation. Defaults to true.
   * @param {boolean} options.ignoreRequired Whether to ignore required properties during validation. Defaults to false.
   * @param {boolean} options.throwError Whether to throw an error if validation fails. Defaults to true.
   * @returns {Object|boolean}
   */
  validate (data, options = { useDefaults: true, ignoreRequired: false, throwError: true }) {
    if (!this.framework.isSchemaVersionGte('1.0.0')) {
      throw new Error('Validation is only supported for schema version 1.0.0 and above')
    }
    const ajvSchema = this.parent.ajvSchemas.getSchema(this.name)
    if (!ajvSchema) {
      const err = new Error(`No AJV schema found for ${this.name}`)
      err.number = 10016
      throw err
    }
    try {
      const withDefaults = ajvSchema.validate(data, { useDefaults: options.useDefaults, ignoreRequired: options.ignoreRequired })
      return withDefaults
    } catch (err) {
  ...
  /**
   * Get the built AJV schema for this schema.
   * @returns {Object}
   */
  get built () {
    if (!this.framework.isSchemaVersionGte('1.0.0')) {
      throw new Error('AJV schemas are only supported for schema version 1.0.0 and above')
    }
    const ajvSchema = this.parent.ajvSchemas.getSchema(this.name)
    if (!ajvSchema) {
      const err = new Error(`No AJV schema found for ${this.name}`)
  ...
  applyDefaults (output = {}, schemaPath, options = { fillObjects: true }) {
    if (this.framework.isSchemaVersionGte('1.0.0')) {
      const propPath = _.toPath(schemaPath)
      const interimData = _.set({}, propPath, _.cloneDeep(output))
      const defaultedData = this.parent.ajvSchemas.getSchema(this.name).validate(interimData, { ignoreRequired: true })
      return _.get(defaultedData, propPath)
    }
