...
class Schemas {
  ...
  load () {
    ...
    const createSchema = (plugin, filePath) => {
      ...
      if (this.framework.isSchemaVersionGte('1.0.0')) {
        ...
        const inferredSchemaName = json.$anchor ?? path.parse(filePath).name.split('.')[0]
  ...
  /**
   * Finds any schema (model or extension) by name, regardless of type.
   * Unlike getModelSchemaByName, this also matches ExtensionSchema instances
   * (e.g. patch-only schemas like assessment-course/trickle-article).
   * @param {string} schemaName
   * @returns {Schema}
   */
